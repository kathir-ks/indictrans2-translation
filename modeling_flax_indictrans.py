...
# reference:
# https://github.com/kathir-ks/setu-translate/blob/master/stages/tlt_pipelines/flax/modeling_flax_indictrans.py
...
class FlaxIndicTransSinusoidalPositionalEmbedding(nn.Module):
    ...
    def __call__(
        ...
        if max_pos > self.weights.shape[0]:
            weights = self._make_weights(max_pos + self.offset, self.embedding_dim, self.weights, self.padding_idx)
        else:
            weights = self.weights

        return weights[position_ids.ravel()].reshape(bsz, seq_len, -1)
